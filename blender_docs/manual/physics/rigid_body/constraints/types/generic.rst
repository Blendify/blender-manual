...
Clamping the min/max to zero has the same effect as the Point_ constraint.

Clamping the relative rotation to zero keeps the objects in alignment.
Combining an absolute rotation and translation clamp would behave much like the Fixed_ constraint.
