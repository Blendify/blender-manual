...
auto-computed ones. It is mostly used in the game-dev area, where it allows to counter-balance some issues generated
by low-poly objects (the most famous examples are the low-poly trees/bushes/grass/etc., and the 'rounded' corners).
...
.. note::
   ...
   Please note that once you have custom normals, the angle threshold of the *Auto Smooth* behavior is disabled --
...
Some tools, in particular CAD ones, tends to generate awful ugly geometries
...
.. note::
   ...
   Currently, only FBX importer is capable of importing custom normals.
